<template>
  <section>
    ...
    <div class="info articleDetailBox">
      <p>
        本文于 {{ toTime(article.update_at, '/') }} 下午 发布在，当前已被围观 {{
          article.meta.views
        }} 次
      </p>
      <p>
      ...
      <p>作者：Gauss</p>
      <p>链接：http://yuangauss287.top/article/{{ article._id }}</p>
    ...
    <transition name="fade">
      <div class="shearBox articleDetailBox" v-show="isShowShear" style="background: transparent">
        <shear
          :title="article.title"
          :url="`http://yuangauss287.top/article/${article._id}`"
        ></shear>
      </div>
    </transition>
      ...
      <div class="arcCommentList">
        <div class="arcCommentItem" v-for="(comment, index) in comments" :key="index">
          <div class="userAverter">
            <img
              :src="comment.author.gravatar ? avarterArr[comment.author.gravatar] : avarterArr[0]"
              alt=""
            />
          </div>
          <div class="userComment">
            <div class="userInfo">
              <div class="userInfoMata">
                <a class="name" :href="comment.author.site" target="_blank">{{
                  comment.author.name
                }}</a>
                <span>
                  <i class="iconfont">
                ...
                <span
                  ><i class="iconfont">&#xe662;</i>{{ comment.country }} - {{ comment.city }}</span
                >
              </div>
            </div>
              ...
              <div class="replyConten" v-if="comment.isShow">
                <div class="replayItem" v-for="(replys, j) in comment.replyList" :key="j">
                  ...
                  <div class="replayItemContet">
                    <div class="userInfoMata">
                      <a class="name" :href="replys.from.site" target="_blank">{{
                        replys.from.name
                      }}</a>
                      <span>
                        <i class="iconfont">
                      ...
                      <span><i class="iconfont">&#xe68f;</i>{{ getBrowser(comment.agent) }}</span>
                      <span
                        ><i class="iconfont">&#xe662;</i>{{ replys.country }} - {{
                          replys.city
                        }}</span
                      >
                    </div>
                    <div class="replyPepole" v-if="replys.to.name">
                      回复 <a :href="replys.to.site" target="_blank">{{ replys.to.name }}：</a>
                    </div>
                    <div class="rContent">{{ replys.content }}</div>
                    <div class="rTime">
                      <span>{{ getDateDiff(comment.update_at) }}</span>
                      <span class="toShowReply" @click="toReplyComment(replys.from, index)"
                        >回复</span
                      >
                    </div>
                  </div>
...
export default {
  ...
  head() {
    return {
      title: this.article.title || `Gauss | blog`
    };
  },
  validate({ params }) {
    return params.id;
