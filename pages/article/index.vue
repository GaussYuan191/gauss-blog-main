      ...
      <section class="article">
        <div class="item" v-for="(item, index) in articles" :key="index">
          ...
          <div class="artice-info">
            ...
            <i class="iconfont">&#xe600;</i>
            <span class="time">
              <strong v-if="item.meta">{{ item.meta.comments }}</strong>
              条评论
            </span>
            <i class="iconfont">&#xe600;</i>
            <span class="time">
              <strong v-if="item.meta">{{ item.meta.likes }} </strong>
              人喜欢
            </span>
      ...
      <section class="side">
        <div class="hot" id="sideHot">
          ...
          <div class="hot-article">
            <h3
              ...
              :key="index"
              @click="goDetail(item._id, hotArticle)"
            >
        ...
        <div :class="showFixedTag ? 'fixedTag' : ''">
          <div class="tags" id="tags">
            ...
            <div class="tag-list">
              <nuxt-link v-for="(item, index) in tags" :key="index" :to="`/article?tag=${item._id}`"
                >{{ item.name }}<span>({{ item.count }})</span></nuxt-link
              >
            </div>
          </div>
          <div class="smallNav">
